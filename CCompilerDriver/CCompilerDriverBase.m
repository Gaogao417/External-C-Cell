...
BaseDriver["OptionsExceptions"]["CreateObjectFile"] := 
	{"ExtraObjectFiles", "Libraries", "LibraryDirectories", "LibraryType", "LinkerOptions", "MprepOptions",
		"SystemLibraries", "SystemLibraryDirectories", "SystemLinkerOptions"}

(* System configuration of interest. *)
Bits[] := Bits[$SystemID]

Bits["Windows-x86-64" | "Linux-x86-64" | "MacOSX-x86-64" | "MacOSX-ARM64"] := 64

Bits[_] := 32

SystemIDBits["Windows-x86-64" | "Linux-x86-64" | "MacOSX-x86-64" | "MacOSX-ARM64"] := 64
...
DeriveOptions[opts_List, exceptions_:{}] := 
	DeleteCases[
		...
		(Rule|RuleDelayed)[Alternatives@@exceptions, _]
	]
...
stripDuplicates[{r : (Rule|RuleDelayed)[x_, y_], xs___}] :=
...
GetOption[driver_, errHd_Symbol, opts_List, n:"TargetDirectory"] := 
	Module[{dir = GetTypeCheckedOption[driver, opts, n,
			_String | Automatic, Message[errHd::tgtdir, #]&]},
		If[dir === Automatic,
			dir = CCompilerDriver`$CCompilerDefaultDirectory];
		If[!QuietDirectoryQ[dir],
...
InvokeCompilerImpl[driver_, compileFn_, errMsgHd_, ext_,
	...
	Module[{outputBaseName, outputFileName, compilerInstallation, compilerName, 
		...
		validInstallationQ = If[compilerName === Automatic,
			driver["ValidInstallationQ"][compilerInstallation]
			,
			driver["ValidInstallationQ"][compilerInstallation, compilerName]
		];

		If[validInstallationQ === False ||
		...
		*)
		...
		linkWithMathLinkQ = driver["LinkWithMathLink"][compileFn] ||
			Length[tmSrcFiles] > 0 || MatchQ[translib, "MathLink" | "WSTP"];
...
Switch[$OperatingSystem,
	...
	"MacOSX",
		BaseDriver["SupportedTargetSystemIDQ"][
			"MacOSX-x86" | "MacOSX-x86-64" | "MacOSX-ARM64", ___] := True
]
...
ResolveLibraries[driver_, Automatic, 
	target: "MacOSX-x86-64" | "MacOSX-ARM64", False, translib_] :=(
...
BaseDriver["ResolveSystemLibraries"][targetSystemID_, translib_] :=
	TransferProtocolLibrary[targetSystemID, translib]
...
TransferProtocolLibrary[sysid_, translib_] :=
	If[translib === "WSTP",
		Switch[sysid,
			"Windows-x86-64",
				"wstp64i4m.lib",
			...
			"MacOSX-x86-64" | "MacOSX-ARM64",
				"-framework wstp",
			"Linux" | "Windows" | "MacOSX-x86",
				{},
			$Failed,
				{},
			_,
				Message[TransferProtocolLibrary::system, sysid, translib]; {}
		]
		,
		Switch[sysid,
			"Windows-x86-64",
			...
			$Failed,
				{},
			_,
				Message[TransferProtocolLibrary::system, sysid, "MathLink"]; {}
		]
	]
...
ResolveLibraryDirectories[driver_, errHd_Symbol, Automatic, sysid_, opts:OptionsPattern[]] :=
	Module[{translib, mlDir, wolframRTLDir},
		...
		If[MatchQ[sysid, "MacOSX-x86-64" | "MacOSX-ARM64"], mlDir = "-F" <> mlDir];
...
ValidSystemIDQ[system_] := 
	system === $SystemID (* $SystemID is correct by definition *) ||
		MatchQ[system,
			...
			"MacOSX-x86" | "MacOSX-x86-64" | "MacOSX-ARM64"
		]
...
MacOSXVersionMinFlag[sysid_ : $SystemID] := Module[{version},
	version = Environment["MACOSX_DEPLOYMENT_TARGET"];
	If[!StringQ[version],
		version = Switch[sysid,
			"MacOSX-ARM64",
				"11.0",
	...
	StringJoin["-mmacosx-version-min=", version]
] /; $OperatingSystem === "MacOSX"

MacOSXVersionMinFlag[___] := ""
