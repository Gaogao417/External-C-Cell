...
$CompilerOptions = {"SystemCompileOptions" :> {"-fPIC", $OptFlag}};

Options[$ThisDriver] = DeriveOptions[$CompilerOptions];
...
Switch[$SystemID,
	"MacOSX-x86-64",
		GCCCompiler["SupportedTargetSystemIDQ"]["MacOSX-ARM64", ___] := False;
	,
	"MacOSX-ARM64",
		GCCCompiler["SupportedTargetSystemIDQ"]["MacOSX-x86" | "MacOSX-x86-64", ___] := False;
	,
	_,
		Null
]
...
GCCCompiler["CreateObjectFileCommands"][
	...
	Module[{},
		...
		CommandJoin[
			...
			" ", compilerArchitectureOption[targetSystemID],
			" ", MacOSXVersionMinFlag[targetSystemID],
...
GCCCompiler["CreateExecutableCommands"][
	errHd_, installation_, compilerName_, outFile_, workDir_, 
	compileOptions_, linkerOptionsIn_, defines_, includePath_, srcFileRules_List, 
	tmSrcFiles_, cFiles_, syslibs_, libs_, libpath_, extraObjects_, 
	targetSystemID_, cleanIntermediate_, mprepOptions_, translib_, language_, opts_] :=
	Module[{libDir, linkerOptions},
		libDir = FileNameJoin[{$InstallationDirectory, "SystemFiles", "Libraries", $SystemID}];
		linkerOptions = Join[linkerOptionsIn, {"-rpath", QuoteFile[libDir]}];
		CommandJoin[
...
exeLinkLibraries["MacOSX-x86" | "MacOSX-x86-64" | "MacOSX-ARM64"] :=
	"-lm -lpthread -lstdc++ -framework Foundation"
...
dllLinkLibs["MacOSX-x86"] := "-lstdc++ -lgcc_eh -framework Foundation"
dllLinkLibs["MacOSX-x86-64" | "MacOSX-ARM64"] := "-lstdc++ -framework Foundation"
dllLinkLibs[_] := ""

(* Determine the target architecture for compilation *)
compilerArchitectureOption["Linux-ARM"] = ""
compilerArchitectureOption["MacOSX-x86"] /; $SystemID === "MacOSX-x86-64" = "-arch i386";
compilerArchitectureOption["MacOSX-x86-64"] /; $SystemID === "MacOSX-x86-64" = "-arch x86_64";
compilerArchitectureOption["MacOSX-ARM64"] = ""; (* gcc is experimental on ARM64 and does not support -arch for now *)

compilerArchitectureOption[target_] := compilerArchitectureOption[Bits[target]]
compilerArchitectureOption[b:(32|64)] := "-m"<>ToString[b]
